...
All API requests and the MCP server require a Bearer token. Most API keys are used with **per-business** routes (`/api/v1/businesses/{businessId}/...`) and configurable scopes. **Creating a new business** uses `POST /api/v1/businesses` (no `businessId` in the path) and requires the **`businesses:write`** scope — use the **Agency / onboarding automation** preset or add that scope manually; it is **not** included in the default Chrome Extension preset.
...
| `businesses:write` | **Create new businesses** under your agency (`POST /api/v1/businesses`) — agency owner, superuser, or admin manager only |
